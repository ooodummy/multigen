...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data) {
    ...
    if (!g_pVB || g_VertexBufferSize < draw_data->TotalVtxCount) {
        ...
        desc.MiscFlags = 0;
        SUCCEDED(g_pd3dDevice->CreateBuffer(&desc, NULL, &g_pVB))
    }
    if (!g_pIB || g_IndexBufferSize < draw_data->TotalIdxCount) {
        ...
        desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
        SUCCEDED(g_pd3dDevice->CreateBuffer(&desc, NULL, &g_pIB))
    }
...
static void ImGui_ImplDX11_CreateFontsTexture() {
    ...
    // Upload texture to graphics system
    {
        ...
        SUCCEDED(g_pd3dDevice->CreateTexture2D(&desc, &subResource, &pTexture))
        // Create texture view
        ...
        SUCCEDED(g_pd3dDevice->CreateShaderResourceView(pTexture, &srvDesc, &g_pFontTextureView))
        pTexture->Release();
    }
    ...
    // Create texture sampler
    {
        ...
        desc.MinLOD = 0.f;
        desc.MaxLOD = 0.f;
        SUCCEDED(g_pd3dDevice->CreateSamplerState(&desc, &g_pFontSampler))
    }
}
